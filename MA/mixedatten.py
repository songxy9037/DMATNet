...
from utilss.box_ops import box_xyxy_to_cxcywh
from utilss.misc import is_main_process
...
class Attention(nn.Module):
    ...
    def forward_conv(self, x, t_h, t_w, s_h, s_w):
        ...
        if self.conv_proj_q is not None:
            t_q = self.conv_proj_q(template)    # b c h w -> b (h w) c
    ...
    def forward(self, x, t_h, t_w, s_h, s_w):
        """
        Asymmetric mixed attention. 输入是一个一维的
        """
        if (
